...
const ReviewCard = ({ user }) => {
  ...
  return (
    <div className="rounded-lg mx-auto cursor-pointer reviewCard px-6 md:px-6 sm:px-4 py-4 md:space-y-3.5 sm:space-y-2.5 space-y-3.5 transition-all max-w-[23rem] shadow-sm duration-300">
      ...
      {/* Review Text */}
      <p className="text-sm text-justify  sm:text-xs md:text-sm tracking-normal">
        {" "}
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Repellat
        veritatis, aut voluptates officia ipsa modi .
      </p>

      {/* Review Stars */}
      <footer className="text-lg space-x-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star key={star} color="warning" fontSize="inherit" />
        ))}
      </footer>
    </div>
  );
...
export const ReviewCardSkeleton = () => (
  <div className="rounded-lg mx-auto px-6 py-4 space-y-3.5 max-w-[23rem] sm:shadow-md shadow-sm">
    <header className="flex items-center space-x-3">
      {/* Img */}
      <Skeleton height={45} width={45} variant="circular" />

      <div>
        {/* Name */}
        <Skeleton height={33} width={85} variant="text" />

        {/* Title */}
        <Skeleton height={15} width={100} variant="text" />
      </div>
    </header>
    {/* Review Text */}
    <div>
      ...
      <Skeleton height={15} width={"100%"} variant="text" />
      <Skeleton height={15} width={"100%"} variant="text" />
      <Skeleton height={15} width={"40%"} variant="text" />
    </div>

    {/* Review Stars */}
    <footer className="text-lg flex space-x-2">
      {[1, 2, 3, 4, 5].map((star) => (
        <Skeleton key={star} height={13} width={13} variant="circular" />
      ))}
    </footer>
  </div>
);
